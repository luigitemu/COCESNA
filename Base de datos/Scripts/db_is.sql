...
CREATE TABLE IF NOT EXISTS `db_is`.`personal` (
  ...
  PRIMARY KEY (`id_personal`, `id_posicion`, `id_turno`),
  INDEX `fk_personal_posicion_idx` (`id_posicion` ASC) VISIBLE,
  INDEX `fk_personal_turnos1_idx` (`id_turno` ASC) VISIBLE,
  CONSTRAINT `fk_personal_posicion`
    FOREIGN KEY (`id_posicion`)
    REFERENCES `db_is`.`posicion` (`id_posicion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_personal_turnos1`
